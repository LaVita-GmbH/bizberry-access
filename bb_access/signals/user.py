...
from bb_access.media_sender import Sender, Content
from bb_access import models


@receiver(post_save, sender=models.User)
def user_post_save_receiver(
    sender, instance: models.User, created: bool = False, **kwargs
):
    if not created:
        return
    ...
    if ... and not instance.has_usable_password():
        instance.request_otp(
            type=models.UserOTP.UserOTPType.PIN, length=12, validity=timedelta(hours=72)
        )


@receiver(post_save, sender=models.UserOTP)
def user_otp_post_save_receiver(
    sender, instance: models.UserOTP, created: bool = False, **kwargs
):
    if not created:
        return
    ...
    MessageSender = Sender.get_sender(
        "SMS"
