...
from bb_access.models import Scope, Role
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: CommandParser):
        parser.add_argument(
            "--file",
            type=str,
            default=os.path.join(settings.BASE_DIR, "access.yml"),
            required=False,
        )

    @classmethod
    def _get_attribute_with_fallbacks(
        cls, key: str, objects: List[dict], fallback: Any = None
    ):
        for obj in objects:
            try:
                return obj[key]
            ...
            for resource in service["resources"]:
                for action in resource["actions"]:
                    ...
                    for selector in selectors:
                        ...
                        scope.is_internal = self._get_attribute_with_fallbacks(
                            "internal", [selector, action, resource, service], False
                        )
                        scope.is_critical = self._get_attribute_with_fallbacks(
                            "critical", [selector, action, resource, service], False
                        )

                        scope.save()
                        current_scopes.append(scope)

        old_scopes = Scope.objects.exclude(
            id__in=[scope.id for scope in current_scopes]
        )
    ...
    def sync_roles(self, roles):
        ...
        for role in roles:
            ...
            for scope in role.get("scopes", []):
                service, resource, action, selector = (
                    scope.get("code").split(".") + [None]
                )[:4]
                try:
                    scope_obj = Scope.objects.get(
                        service=service,
                        resource=resource,
                        action=action,
                        selector=selector,
            ...
            for included_role in role.get("included_roles", []):
                included_role_objs.append(
                    Role.objects.get(name=included_role.get("key"))
                )
