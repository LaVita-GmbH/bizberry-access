...
class Content(BaseModel):
    ...
    def _jinja_env(self):
        ...
        env.filters["format_date"] = lambda s: format_date(
            datetime.fromisoformat(s).date(), locale=locale
        )
        env.filters["format_datetime"] = lambda s: format_datetime(
            datetime.fromisoformat(s), locale=locale
        )
        env.filters["format_currency"] = lambda f, currency: format_currency(
            f, currency, "#,##0.00 ¤", locale=locale
        )
    ...
    def _template_render(self) -> str:
        with open(
            os.path.join(settings.BASE_DIR, "bb_access", "templates", self.template),
            "rb",
        ) as template:
            template: jinja2.Template = self._jinja_env.from_string(
                str(template.read(), "utf-8")
            )
    ...
    def get_sender(cls, medium: str):
        for scls in cls.__subclasses__():
            ...
            for integration in scls.__subclasses__():
                if integration.INTEGRATION == getattr(
                    settings, f"SENDER_{medium}_INTEGRATION"
                ):
                    return integration
