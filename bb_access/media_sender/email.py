...
class SMTPEmailSender(EmailSender):
    ...
    def _get_message(self):
        ...
        message["From"] = Address(
            settings.SENDER_EMAIL_INTEGRATION_SMTP_SENDER_NAME,
            addr_spec=settings.SENDER_EMAIL_INTEGRATION_SMTP_SENDER_EMAIL,
        )
        message["To"] = self.content.receiver
        message["Message-ID"] = (
            f"<{random_string_generator(size=64)}@{socket.getfqdn()}>"
        )
    ...
    def _send_message(self, message):
        ...
        if (
            settings.SENDER_EMAIL_INTEGRATION_SMTP_USE_SSL
            and not settings.SENDER_EMAIL_INTEGRATION_SMTP_USE_STARTTLS
        ):
        ...
        with connection(
            ...
            if (
                settings.SENDER_EMAIL_INTEGRATION_SMTP_USER
                and settings.SENDER_EMAIL_INTEGRATION_SMTP_PASSWORD
            ):
                smtp.login(
    ...
    def send(self):
        while True:
            try:
                ...
                if isinstance(error.__cause__, UnicodeEncodeError):
                    self.content.receiver = unidecode(
                        self.content.receiver.replace("ä", "ae")
                        .replace("ö", "oe")
                        .replace("ü", "ue")
                    )
