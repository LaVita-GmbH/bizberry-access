...
def _get_tenant_countries_filtered(
    tenant: Tenant, pagination: Pagination
) -> List[TenantCountry]:
    return list(tenant.countries.all()[pagination.offset : pagination.limit])


def _create_tenant_country(
    tenant: Tenant, body: request.TenantCountryCreate
) -> TenantCountry:
...
def get_tenants(
    ...
    return response.TenantsList(
        tenants=[response.Tenant.from_orm(tenant) for tenant in tenants],
    )
...
def get_tenant_countries(
    ...
    return response.TenantCountriesList(
        countries=[response.TenantCountry.from_orm(country) for country in countries],
    )
